...
#########################################################
print(
    "STEP 0. We get the information about MemoryStoreRetentionPeriodInHours settings in the table"
)
try:
    ...
    print(
        f"For the table {TABLE_NAME}, MemoryStoreRetentionPeriodInHours = {hours_retention_memory}"
    )
...
#########################################################
print(
    "STEP 1. prepare 1 record to insert. Record_time fits in MemoryStoreRetentionPeriodInHours interval"
)
hours_old = (
    hours_retention_memory - 1
)  # 1 hour less than MemoryStoreRetentionPeriodInHours
...
#########################################################
print(
    "STEP 2. prepare 1 record to insert. Record_time DOESN'T fit in MemoryStoreRetentionPeriodInHours interval"
)
hours_old = (
    hours_retention_memory + 1
)  # 1 hour more than MemoryStoreRetentionPeriodInHours
records = prepare_records(hours_old)
try:
    writer = TimestreamTableWriter(DB_NAME, TABLE_NAME, ts_client)
    writer.write_records_simple(records)
